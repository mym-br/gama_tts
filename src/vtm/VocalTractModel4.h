...
#define GS_VTM4_INPUT_FILTER_PERIOD_SEC (50.0e-3)
...
#define GS_VTM4_OUTPUT_SCALE              0.95
...
VocalTractModel4<FloatType, SectionDelay>::parseInputStream(std::istream& in)
{
	...
	while (std::getline(in, line)) {
		...
		for (int i = 0; i < TOTAL_REGIONS; ++i) {
			value[PARAM_R1 + i] = std::max(value[PARAM_R1 + i] * config_.radiusCoef[i], FloatType{GS_VTM4_MIN_RADIUS});
		}
...
VocalTractModel4<FloatType, SectionDelay>::initializeSynthesizer()
{
	...
	if (interactive_) {
		inputFilters_ = std::make_unique<InputFilters>(sampleRate_, GS_VTM4_INPUT_FILTER_PERIOD_SEC);
	}
}
...
VocalTractModel4<FloatType, SectionDelay>::synthesize()
{
	...
	signal = vocalTract(((pulse + (ah1 * signal)) * FloatType{GS_VTM4_VT_SCALE}),
				bandpassFilter_->filter(signal));

	/*  PUT PULSE THROUGH THROAT  */
	signal += throat_->process(pulse * FloatType{GS_VTM4_VT_SCALE});
...
VocalTractModel4<FloatType, SectionDelay>::calculateOutputScale()
{
	...
	const float scale = GS_VTM4_OUTPUT_SCALE / maxValue;
	LOG_DEBUG("\nScale: " << scale << '\n');
	return scale;
}
