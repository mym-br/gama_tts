...
#define GS_VTM_VOCAL_TRACT_MODEL_2_MIN_RADIUS (0.001)
#define GS_VTM_VOCAL_TRACT_MODEL_2_INPUT_FILTER_PERIOD_SEC (50.0e-3)

/*  SCALING CONSTANT FOR INPUT TO VOCAL TRACT & THROAT (MATCHES DSP)  */
//#define GS_VTM_VOCAL_TRACT_MODEL_2_VT_SCALE                  0.03125     /*  2^(-5)  */
// this is a temporary fix only, to try to match dsp synthesizer
#define GS_VTM_VOCAL_TRACT_MODEL_2_VT_SCALE                  0.125     /*  2^(-3)  */

/*  FINAL OUTPUT SCALING, SO THAT .SND FILES APPROX. MATCH DSP OUTPUT  */
#define GS_VTM_VOCAL_TRACT_MODEL_2_OUTPUT_SCALE              0.95
...
public:
	VocalTractModel4(const ConfigurationData& data, bool interactive = false);
	~VocalTractModel4() {}
...
template<typename FloatType, unsigned int SectionDelay>
VocalTractModel4<FloatType, SectionDelay>::VocalTractModel4(const ConfigurationData& data, bool interactive)
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::loadConfiguration(const ConfigurationData& data)
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::reset()
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::synthesizeToFile(std::istream& inputStream, const char* outputFile)
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::synthesizeToBuffer(std::istream& inputStream, std::vector<float>& outputBuffer)
{
	if (!outputData_.empty()) {
		reset();
	}
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::parseInputStream(std::istream& in)
{
	std::string line;
	...
	while (std::getline(in, line)) {
		...
		if (!lineStream) {
			THROW_EXCEPTION(VTMException, "[VTM::VocalTractModel4] Error in input parameter parsing: Could not read parameters (line number " << lineNumber << ").");
		}

		// R1 - R8.
		for (int i = 0; i < TOTAL_REGIONS; ++i) {
			value[PARAM_R1 + i] = std::max(value[PARAM_R1 + i] * config_.radiusCoef[i], FloatType{GS_VTM_VOCAL_TRACT_MODEL_2_MIN_RADIUS});
		}
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::initializeSynthesizer()
{
	FloatType nyquist;
	...
	if (config_.length > 0.0) {
		...
		LOG_DEBUG("[VocalTractModel4] Internal sample rate: " << sampleRate_);
	} else {
	...
	if (interactive_) {
		inputFilters_ = std::make_unique<InputFilters>(sampleRate_, GS_VTM_VOCAL_TRACT_MODEL_2_INPUT_FILTER_PERIOD_SEC);
	}
}
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::synthesizeForInputSequence()
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::synthesizeSamples(std::size_t numberOfSamples)
{
	if (!inputFilters_) {
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::synthesize()
{
	...
	signal = vocalTract(((pulse + (ah1 * signal)) * FloatType{GS_VTM_VOCAL_TRACT_MODEL_2_VT_SCALE}),
				bandpassFilter_->filter(signal));

	/*  PUT PULSE THROUGH THROAT  */
	signal += throat_->process(pulse * FloatType{GS_VTM_VOCAL_TRACT_MODEL_2_VT_SCALE});
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::initializeNasalCavity()
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::calculateTubeCoefficients()
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::setFricationTaps()
{
...
template<typename FloatType, unsigned int SectionDelay>
FloatType
VocalTractModel4<FloatType, SectionDelay>::vocalTract(FloatType input, FloatType frication)
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::writeOutputToFile(const char* outputFile)
{
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::writeOutputToBuffer(std::vector<float>& outputBuffer)
{
	/*  BE SURE TO FLUSH SRC BUFFER  */
	srConv_->flushBuffer();
...
template<typename FloatType, unsigned int SectionDelay>
float
VocalTractModel4<FloatType, SectionDelay>::calculateOutputScale()
{
	...
	const float scale = GS_VTM_VOCAL_TRACT_MODEL_2_OUTPUT_SCALE / maxValue;
	LOG_DEBUG("\nScale: " << scale << '\n');
	return scale;
}
...
template<typename FloatType, unsigned int SectionDelay>
void
VocalTractModel4<FloatType, SectionDelay>::loadSingleInput(const VocalTractModelParameterValue pv)
{
	switch (pv.index) {
	...
	case PARAM_R8:
		singleInput_[pv.index] = std::max(
					...
					FloatType{GS_VTM_VOCAL_TRACT_MODEL_2_MIN_RADIUS});
		break;
	default:
...
template<typename FloatType, unsigned int SectionDelay>
std::size_t
VocalTractModel4<FloatType, SectionDelay>::getOutputSamples(std::size_t n, float* buffer)
{
